...
class ThirdForm:
    def uploadAadhaarForm(self):
        ws = Tk()
        ws.title('Upload Aadhaar Card')
        ws.config(bg='#0B5A81')
        ...
        def browse_aadhaar():
            ...
            def upload_aadhar():
                ...
                if not db.child(username).shallow().get().val():
                    ...
                    path_on_cloud = username+"/aadhaar.png"
                    path_on_local = filename
                    storage.child(path_on_cloud).put(path_on_local)

                    # saving to processing directory
                    storage.child(path_on_cloud).download("aadharCard.png")
                    ...
                    print(user)
                    image_url = storage.child(username).child("aadhaar.png").get_url(user['idToken'])
                    print("this is image url: ",image_url)
                    db.child(username).child("aadhaar_url").set(image_url)
                    subject = "Aadhaar Card has been uploaded."
                    message = "Congratulations! " + username + ",\nYou have successfully uploaded you Aadhaar Card to " \
                                                               "our server\n Sit tight and relax while we process " \
                                                               "your details. \nRegards,\nTeam AutoKYC "
                    messagebox.showinfo("Registration Successful!", "Aadhaar Card submission Successful!\nYou will "
                                                                    "receive "
